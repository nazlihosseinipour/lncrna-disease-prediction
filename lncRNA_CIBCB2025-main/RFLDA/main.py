...
for fold in range(n_folds):
    ...
    pd.DataFrame(y_test_pred, columns = y.columns ).to_csv(dataset + "RFLDA_" + mode +"_predictions_test_fold" + str(fold + 1) + ".csv", index=False) 
    model.accuracy_find_best_n.to_csv(dataset + "accuracy_" + mode + "_find_best_n_" + str(fold + 1) + ".csv", index=False)
    fold +=1
