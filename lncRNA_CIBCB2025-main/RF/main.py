...
for fold in range(n_folds):
    ...
    x_test = test[fold][0]
    y_test = test[fold][1]
    model = RF(binary_mode=binary_mode,
    ...
    print(mode)
    y_test_pred = np.array(model.predict_proba(x_test, y_test))
    pd.DataFrame(y_test_pred, columns=y.columns).to_csv(dataset + "RF_" + mode + "_predictions_test_fold" + str(fold + 1) + ".csv", index=False)
    fold += 1
