...
for direction in v1:v2 v2:v1; do
  ...
  for feature in "${features[@]}"; do
    ...
    run_cell "$cell" "$output" python scripts/run_inductive_transfer_experiments.py --source-version "$source" --target-version "$target" --models rflda --feature-keys "$feature" --label-space both --shared-disease-list results/audit/disease_protocol/canonical_shared_disease_list.csv --threshold-mode fixed --skip-target-cv --outdir "$outdir"
  done
done
