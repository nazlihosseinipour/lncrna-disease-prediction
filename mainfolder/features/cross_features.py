from mainfolder.core.feature_module import FeatureModule
import numpy as np
import pandas as pd
...
class CrossFeatures(FeatureModule):
    ...
    METHOD_MAP = {
        16: "calculate_gip_lncRNA_and_dis",
        17: "extract_svd_features",
    }
    ...
    @staticmethod
    def calculate_gip_lncRNA_and_dis(matrix: MatrixLike) -> Tuple[pd.DataFrame, pd.DataFrame]:
        """
        Returns (GIP_lncRNA, GIP_disease):
          - GIP_lncRNA: kernel over rows (lncRNAs)
          - GIP_disease: kernel over columns (diseases)
        """
        A, row_labels, col_labels = CrossFeatures._as_array_and_labels(matrix)
        ...
        gamma_r = CrossFeatures._safe_gamma(norms_sq_r)
        ...
        gamma_c = CrossFeatures._safe_gamma(norms_sq_c)
